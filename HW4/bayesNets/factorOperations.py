...
def joinFactors(factors):
    ...
    for factor in factors:
        unconditionedVariables = unconditionedVariables.union(
            factor.unconditionedVariables())
        conditionedVariables = conditionedVariables.union(
            factor.conditionedVariables())
        conditionedVariables = set([
            conditionedVariable for conditionedVariable in conditionedVariables
            if conditionedVariable not in unconditionedVariables
        ])
...
def eliminateWithCallTracking(callTrackingList=None):
    def eliminate(factor, eliminationVariable):
        ...
        reducedFactor = Factor(unconditionedVariables,
                               factor.conditionedVariables(),
                               factor.variableDomainsDict())

        for assignment in reducedFactor.getAllPossibleAssignmentDicts():
            prob = 0
            for eliminationVal in factor.variableDomainsDict()[
                    eliminationVariable]:
                fullAssignment = assignment
                fullAssignment[eliminationVariable] = eliminationVal

                prob += factor.getProbability(fullAssignment)
            reducedFactor.setProbability(assignment, prob)
