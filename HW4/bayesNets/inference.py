...
def inferenceByEnumeration(bayesNet, queryVariables, evidenceDict):
    ...
    eliminationVariables = (
        bayesNet.variablesSet() - evidenceVariablesSet) - queryVariablesSet
...
def sampleFromFactorRandomSource(randomSource=None):
    ...
    def sampleFromFactor(factor, conditionedAssignments=None):
        ...
        probability.
        """
        if conditionedAssignments is None and len(
                factor.conditionedVariables()) > 0:
            raise ValueError, (
            ...
            if not conditionedVariables.issuperset(
                    set(factor.conditionedVariables())):
        ...
        assignmentDicts = sorted([
            assignmentDict
            for assignmentDict in CPT.getAllPossibleAssignmentDicts()
        ])
        assignmentDictProbabilities = [
            CPT.getProbability(assignmentDict)
