...
def constructBayesNet(gameState):
    ...
    variableDomainsDict[FOOD_HOUSE_VAR] = [
        TOP_LEFT_VAL, TOP_RIGHT_VAL, BOTTOM_LEFT_VAL, BOTTOM_RIGHT_VAL
    ]
    variableDomainsDict[GHOST_HOUSE_VAR] = [
        TOP_LEFT_VAL, TOP_RIGHT_VAL, BOTTOM_LEFT_VAL, BOTTOM_RIGHT_VAL
    ]
...
def fillHouseCPT(bayesNet, gameState):
    foodHouseFactor = bn.Factor([FOOD_HOUSE_VAR], [X_POS_VAR, Y_POS_VAR],
    ...
    ghostHouseFactor = bn.Factor([GHOST_HOUSE_VAR], [X_POS_VAR, Y_POS_VAR],
                                 bayesNet.variableDomainsDict())
...
def fillObsCPT(bayesNet, gameState):
    ...
    def getHouseVal(x, y):
        width = gameState.data.layout.width / 2.0
        height = gameState.data.layout.height / 2.0
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                ...
                blueProb = 0
                noProb = 0
                if assignment[FOOD_HOUSE_VAR] == getHouseVal(
                        obsPos[0], obsPos[1]):
                    redProb = PROB_FOOD_RED
                    blueProb = 1 - PROB_FOOD_RED
                    noProb = 0
                elif assignment[GHOST_HOUSE_VAR] == getHouseVal(
                        obsPos[0], obsPos[1]):
                ...
                if assignment[obsVar] == BLUE_OBS_VAL:
                    ...
                    obsFactor.setProbability(assignment, noProb)
...
class BayesAgent(game.Agent):
    ...
    def getRandomAction(self, gameState):
        ...
        successors = [
            gameState.generatePacmanSuccessor(a).getPacmanPosition()
            for a in legal
        ]
    ...
    def goToBest(self, gameState):
        ...
        bestFoodAssignment = getMostLikelyFoodHousePosition(
            evidence, self.bayesNet, eliminationOrder)

        tx, ty = dict(
            zip([
                BOTTOM_LEFT_VAL, TOP_LEFT_VAL, BOTTOM_RIGHT_VAL, TOP_RIGHT_VAL
            ], gameState.getPossibleHouses()))[bestFoodAssignment[
                FOOD_HOUSE_VAR]]
        bestAction = None
        bestDist = float("inf")
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        leftExpectedValue = CPT.getProbability(
            topLeft) * WON_GAME_REWARD + CPT.getProbability(
                topRight) * GHOST_COLLISION_REWARD
        rightExpectedValue = CPT.getProbability(
            topLeft) * GHOST_COLLISION_REWARD + CPT.getProbability(
                topRight) * WON_GAME_REWARD
    ...
    def getExplorationProbsAndOutcomes(self, evidence):
        ...
        houseMarginals = inference.inferenceByVariableElimination(
            self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], evidence,
            eliminationOrder)
        ...
        for foodHouseVal, ghostHouseVal in [(TOP_LEFT_VAL, TOP_RIGHT_VAL),
            ...
            oneObsMarginal = inference.inferenceByVariableElimination(
                self.bayesNet, [firstUnk], condEvidence,
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for (prob, evidence) in probsandoutcomes:
            expectedValue += prob * max(
    ...
    def exploreAction(self, gameState):
        ...
        evidence = self.getEvidence(gameState)
        enterLeftValue, enterRightValue = self.computeEnterValues(
            evidence, [X_POS_VAR, Y_POS_VAR])
